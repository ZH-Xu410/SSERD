...
class Generator(nn.Module):
    ...
    def get_trainable_params(self):
        return list(self.convs[-4:].parameters()) + list(self.to_rgbs[-2:].parameters())
...
class ExpertDiscriminator(nn.Module):
    def __init__(self,  emotions, actors, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], img_channel=3):
        super().__init__()
        for emo in emotions:
            for act in actors:
                discriminator = Discriminator(size, channel_multiplier, blur_kernel, img_channel)
                self.add_module(f'disc_{emo}_{act}', discriminator)
    
    def forward(self, inputs, emotions, actors):
        out = []
        for x, emo, act in zip(inputs, emotions, actors):
            out.append(getattr(self, f'disc_{emo}_{act}')(x.unsqueeze(0)))
