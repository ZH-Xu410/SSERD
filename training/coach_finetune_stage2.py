...
from datasets.mead_paired_dataset import MEADPairedDataset
...
class Coach:
    def __init__(self, opts):
        ...
        self.net.fix_encoder()

        # self.gaussian_filter = self.get_gaussian_filter().to(self.device)

        # if os.path.exists(self.opts.mouth_latent_mask):
        #     self.mouth_latent_mask = torch.load(
        #         self.opts.mouth_latent_mask, self.device)
        # else:
        #     self.mouth_latent_mask = None
        ...
        # if self.opts.resume and ckpt is not None:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                # ************************ Data Preparation **************************
                x = batch['img_S'].cuda()
                D1 = batch['img_D1'].cuda()
                D2 = batch['img_D2'].cuda()
                T1 = batch['img_T1'].cuda()
                emo_S = batch['emo_S']
                emo_D = batch['emo_D']
                ...
                codes, feats, first_layer_feats, skip_layer_feats, fusion = self.net.encode(x, use_skip=self.opts.use_skip)

                emotion_latents_D1, codes_D1 = self.net.get_emotion_latents(codes, D1, self.opts.use_skip)
                emotion_latents_D2, codes_D2 = self.net.get_emotion_latents(codes, D2, self.opts.use_skip)
                emotion_latents_S, _ = self.net.get_emotion_latents(codes, x, self.opts.use_skip)

                images_SD1, result_latent_SD1 = self.net.decoder([codes],
                                                         ...
                                                         fusion_block=fusion,
                                                         zero_noise=self.opts.zero_noise,
                                                         editing_w=emotion_latents_D1)
                images_SD2, result_latent_SD2 = self.net.decoder([codes],
                                                         input_is_latent=True,
                                                         randomize_noise=True,
                                                         return_latents=True,
                                                         first_layer_feature=first_layer_feats,
                ...
                images_SS, result_latent_SS = self.net.decoder([codes],
                                                         input_is_latent=True,
                                                         randomize_noise=True,
                                                         return_latents=True,
                                                         first_layer_feature=first_layer_feats,
                                                         ...
                                                         fusion_block=fusion,
                                                         zero_noise=self.opts.zero_noise,
                                                         editing_w=emotion_latents_S)

                SD1 = F.interpolate(images_SD1, (256, 256), mode='bilinear')
                SD2 = F.interpolate(images_SD2, (256, 256), mode='bilinear')
                SS = F.interpolate(images_SS, (256, 256), mode='bilinear')


                # images, latents, emo_latents = self.net(x, D1, D2, resize=True, zero_noise=self.opts.zero_noise,
                #                                         first_layer_feature_ind=self.opts.feat_ind,
                #                                         use_skip=self.opts.use_skip, return_latents=True)

                # SS, _ = self.net(x, x, None, resize=True, zero_noise=self.opts.zero_noise,
                #                  first_layer_feature_ind=self.opts.feat_ind,
                #                  use_skip=self.opts.use_skip, return_latents=False)


                actors = actor_S * 2
                emotions = emo_D * 2
                latents = torch.cat([codes, codes_D1, codes_D2], dim=0)
                emo_latents = torch.cat([emotion_latents_D1, emotion_latents_D2], dim=0)
                
                if self.global_step < self.opts.contrastive_start_step:
                    self.latent_pool.add_latent(emotions, actors, emo_latents)

                if self.opts.use_facial_disc:
                    # pair数据用T，unpair用S
                    lm_S = batch['lm_S'].cuda()
                    lm_D1 = batch['lm_D1'].cuda()
                    lm_D2 = batch['lm_D2'].cuda()
                    lm_T = batch['lm_T'].cuda()

                    T_ = x.clone()
                    lm_T_ = lm_S.clone()
                    for i, flag in enumerate(is_paired):
                        if flag:
                            T_[i] = T1[i]
                            lm_T_[i] = lm_T[i]
                    src_facial_component = self.get_roi_regions(T_, lm_T_)
                    SD1_facial_component = self.get_roi_regions(SD1, lm_D1)
                    SD2_facial_component = self.get_roi_regions(SD2, lm_D2)
                    facial_component = (
                        src_facial_component, SD1_facial_component, SD2_facial_component)
                else:
                    facial_component = None

                # adversarial loss
                if self.opts.adv_lambda > 0:
                    real = torch.cat([D1, D1], dim=0)
                    fake = torch.cat([SD1, SD2], dim=0)
                    loss_dict = self.train_discriminator(
                        real, fake, facial_component)
                else:
                    loss_dict = {}

                # calculate losses
                loss, G_loss_dict, id_logs = self.calc_loss(
                    x, D1, D2, T1, is_paired, SS, SD1, SD2, actors, emotions, latents, emo_latents, facial_component)
                ...
                if self.global_step % self.opts.image_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0):
                    self.parse_and_log_images(
                        id_logs, x, D1, D2, T1, SS, SD1, SD2, is_paired, title='images/train/faces')
                    # self.parse_and_log_images(
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(tqdm(self.test_dataloader)):
            ...
            emo_latents = torch.cat(emo_latents, dim=0)
    ...
    def configure_optimizers(self):
        ...
        params = list(self.net.encoder.fusion.parameters())
    ...
    def configure_datasets(self):
        train_dataset = MEADPairedDataset(self.opts, mode='train')
        test_dataset = MEADPairedDataset(self.opts, mode='train', max_num=100)
        if self.opts.use_wandb:
    ...
    def calc_loss(self, x, D1, D2, T1, is_paired, SS, SD1, SD2, actors, emotions, latents, emo_latents, facial_component=None, masks=None, landmarks=None):
        loss_dict = {}
        loss = 0.0
        id_logs = None
        bs = x.shape[0]

        x2 = torch.cat([x, x, x], dim=0)
        y2 = torch.cat([x, T1, T1], dim=0)
        y_hat = torch.cat([SS, SD1, SD2], dim=0)

        if self.opts.id_lambda > 0:
            loss_id, sim_improvement, id_logs = self.id_loss(y_hat, y2, x2)
        ...
        if self.opts.l2_lambda > 0:
            ...
            for i, flag in enumerate(is_paired):
                if not flag:
                    continue
                loss_l2 += F.mse_loss(SD1[i], T1[i]) + \
                    F.mse_loss(SD2[i], T1[i])
                loss_l2_recon += F.mse_loss(SS, x)
                num += 1
            loss_l2 = loss_l2 / max(num, 1) * self.opts.l2_lambda
            loss_l2_recon = loss_l2_recon / max(num, 1) * self.opts.l2_lambda * 0.5
            loss_dict['loss_l2'] = float(loss_l2)
            loss += loss_l2
            loss_dict['loss_l2_recon'] = float(loss_l2_recon)
            loss += loss_l2_recon
        ...
        if self.opts.lpips_lambda > 0:
            ...
            for i, flag in enumerate(is_paired):
                if not flag:
                    continue
                loss_lpips += self.lpips_loss(SD1[i:i+1], T1[i:i+1]).mean(
                ) + self.lpips_loss(SD2[i:i+1], T1[i:i+1]).mean()
                loss_lpips_recon += self.lpips_loss(SS, x).mean()
            loss_lpips = loss_lpips / max(num, 1) * self.opts.lpips_lambda
            loss_lpips_recon = loss_lpips_recon / max(num, 1) * self.opts.lpips_lambda * 0.5
            loss_dict['loss_lpips'] = float(loss_lpips)
            loss += loss_lpips
            loss_dict['loss_lpips_recon'] = float(loss_lpips_recon)
            loss += loss_lpips_recon


        if self.opts.adv_lambda > 0:  # modified
            y_SS = self.discriminator(SS)
            y_SD1 = self.discriminator(SD1)
            y_SD2 = self.discriminator(SD2)
            loss_g = F.softplus(-torch.cat([y_SS, y_SD1, y_SD2], dim=0)
        ...
        if self.opts.emo_contrastive > 0 and self.global_step >= self.opts.contrastive_start_step:
            ...
            tau = 0.1
            result = []
            for i in range(emo_latents.shape[0]):
                try:
                    w_query = self.latent_pool.query(emotions[i], actors[i])
                except:
                    continue
                sim = cos(
                    emo_latents[i:i+1].repeat(w_query.shape[0], 1, 1), w_query).mean(1).unsqueeze(0)
            ...
            if self.opts.consist_lambda > 0:
                emo_latents_SD1, _ = self.net.get_emotion_latents(latents[:bs], SD1, self.opts.use_skip)
                emo_latents_SD2, _ = self.net.get_emotion_latents(latents[:bs], SD2, self.opts.use_skip)

                emo_latents_SD = torch.cat([emo_latents_SD1, emo_latents_SD2], dim=0)

                result = []
                for i in range(emo_latents_SD.shape[0]):
                  try:
                    w_query = self.latent_pool.query(emotions[i], actors[i])
                  except:
                    continue
                  sim = cos(
                    emo_latents_SD[i:i+1].repeat(w_query.shape[0], 1, 1), w_query).mean(1).unsqueeze(0)
                  value = F.cross_entropy(
                    sim / tau, target=torch.tensor([0], device=self.device))
                  result.append(value)
                if len(result) > 0:
                  loss_consist = torch.stack(
                    result, dim=0).mean() * self.opts.consist_lambda
                else:
                  loss_consist = 0
                loss_dict['loss_consist'] = float(loss_consist)
                loss += loss_consist
        ...
        if facial_component is not None:  # (SD1, SD2)
            ...
            left_eye_pred = torch.cat(
                [facial_component[1][0], facial_component[2][0]], dim=0)
            right_eye_pred = torch.cat(
                [facial_component[1][1], facial_component[2][1]], dim=0)
            mouth_pred = torch.cat(
                [facial_component[1][2], facial_component[2][2]], dim=0)
            ...
            if self.opts.facial_feat_lambda > 0:
                ...
                left_eye_feat_loss = self.facial_feature_loss(
                    [left_eye_fake_feat[0][:bs], left_eye_fake_feat[1][:bs]], left_eye_real_feat)
                right_eye_feat_loss = self.facial_feature_loss(
                    [right_eye_fake_feat[0][:bs], right_eye_fake_feat[1][:bs]], right_eye_real_feat)
                mouth_feat_loss = self.facial_feature_loss(
                    [mouth_fake_feat[0][:bs], mouth_fake_feat[1][:bs]], mouth_real_feat)  # TODO if is_paired
    ...
    def get_gaussian_filter(kernel_size=15, sigma=7, channels=3):
        ...
        gaussian_kernel = (1 / (2 * math.pi * variance)) * \
            torch.exp(-torch.sum((xy_grid - mean) **
                      2, dim=-1) / (2 * variance))
        ...
        gaussian_filter = nn.Conv2d(
            channels, channels, kernel_size, groups=channels, bias=False, padding=kernel_size//2)
    ...
    def blur_mouth(self, images, landmarks):
        ...
        bboxes = torch.cat([mouth_center-mouth_out_size//2,
                           mouth_center+mouth_out_size//2], dim=1).long()
        margin = 15
        ...
        for i, bbox in enumerate(bboxes):
            blured_mouth.append(self.gaussian_filter(
                images[i:i+1, :, bbox[1]-margin:bbox[3]+margin, bbox[0]-margin:bbox[2]+margin]))
            masks[i, :, bbox[1]:bbox[3], bbox[0]:bbox[2]] = 1
        blured_mouth = torch.cat(blured_mouth, dim=0)
        masks = masks.bool()
        blured_imgs[masks] = blured_mouth[:, :,
                                          margin:-margin, margin:-margin].reshape(-1)
    ...
    def parse_and_log_images(self, id_logs, x, D1, D2, T1, SS, SD1, SD2, is_paired, title, subscript=None, display_count=2):
        im_data = []
        ...
        for i in range(display_count):
            cur_im_data = {
                ...
                'target1': common.log_input_image(T1[i], self.opts) if is_paired[i] else Image.fromarray(np.zeros([256, 256, 3], dtype=np.uint8)),
                'recon': common.tensor2im(SS[i]), 
