...
class WBLogger:
    ...
    @staticmethod
    def log_images_to_wandb(x, D1, D2, SD1, SD2, id_logs, prefix, step, opts):
        im_data = []
        column_names = ["Source", "Driving1", "Driving2", "Output1", "Output2"]
        if id_logs is not None:
            column_names.append("ID Diff Output to Target")
        for i in range(len(x)):
            cur_im_data = [
                wandb.Image(common.log_input_image(x[i], opts)),
                wandb.Image(common.log_input_image(D1[i], opts)),
                wandb.Image(common.log_input_image(D2[i], opts)),
                wandb.Image(common.tensor2im(SD1[i])),
                wandb.Image(common.tensor2im(SD2[i])),
            ]
            if id_logs is not None:
